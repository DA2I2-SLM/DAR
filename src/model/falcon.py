...
def load_model(args, model_name_or_path, memory_for_model_activations_in_gb=2, peft_path=None):
    ...
    model = AutoModelForCausalLM.from_pretrained(model_name_or_path, torch_dtype=torch.float16, device_map="auto", token=args.token)
...
class FalconWrapper(object):
    ...
    def generate(self, args, query, max_new_tokens=64, return_logits=True, verbose=False):
        ...
        #     gen_response = gen_response.split('</s>')[0]
        #     gen_response = gen_response.replace('Ġ',' ').replace('ĊĊ','').strip()
